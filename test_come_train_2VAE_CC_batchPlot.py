...
for epoch in range(epochs):
    ...
    train_losses_VAE = np.zeros(3)
    valid_losses_VAE = np.zeros(3)
    ...
    with torch.no_grad():
        for i, (x1, x2) in enumerate(iter(train_loader)):
            ...
            l = computeLoss_VAE(model1, x1, beta)
            for j in range(3):
                valid_losses_VAE[j] += l[j].cpu().numpy()*x1.size()[0]/nb_valid
    ...
    writer.add_scalars("Overview",{
        'Training, VAE' : train_losses_VAE[0],
        'Validation, VAE' : valid_losses_VAE[0],
        'Training, CC' : train_losses_CC[0],
        'Validation, CC' : valid_losses_CC[0]
    }, epoch)

    writer.add_scalars("VAE",{
        'Training, Reconstruction' : train_losses_VAE[1],
        'Validation, Reconstruction' : valid_losses_VAE[1],
        'Training, Kullback-Liebler Divergence' : train_losses_VAE[2],
        'Validation, Kullback-Liebler Divergence' : valid_losses_VAE[2]
    }, epoch)
